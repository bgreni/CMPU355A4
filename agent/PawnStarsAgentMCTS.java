...
/**
 * UCT implementation based on the of this example agent from Ludii
...
public class PawnStarsAgent extends AI
{
	...
	{
		this.friendlyName = "PawnStarsAgent";
	}
	...
	)
	{
		...
		)
		{
			...
			// select a node to run a playout from
			current = findPlayoutNode(current);
			...
			backProbUtils(current, game, utilities);
	...
	private static void backProbUtils(Node current, Game game, double[] utilities) {
	...
	private static Node findPlayoutNode(Node current) {
		// Traverse tree
		while (true)
		{
	...
	/**
	 * TODO: Not actually using a heuristic right now
	 * Currently just picking a randome move
	 */
	private static Move getMoveByHeuristic(Node current){
		return current.unexpandedMoves.remove(
			ThreadLocalRandom.current().nextInt(current.unexpandedMoves.size()));
	}

	private static Node pickUnexpandedNode(Node current) {
		// randomly select an unexpanded move
		final Move move = getMoveByHeuristic(current);
	...
	public static Node select(final Node current)
	{
		...
		{
			return pickUnexpandedNode(current);
		}
