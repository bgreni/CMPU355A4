...
public class PawnStarsAgent extends ExpertPolicy
{
	 ...
	 */
	public static PawnStarsAgent createPawnStarsAgent()
	{
		return new PawnStarsAgent();
	}
	...
	)
	{
		...
		if (numPlayers > 2)
		{
			...
			rootAlphaInit = ((float) AIUtils.rankToUtil(context.state().computeNextLossRank(), numPlayers)) * BETA_INIT;
			rootBetaInit = ((float) AIUtils.rankToUtil(context.state().computeNextWinRank(), numPlayers)) * BETA_INIT;
		}
		else
		{
	...
	)
	{
		...
		if (numPlayers > 2)
		{
			...
			alpha = Math.max(alpha, ((float) AIUtils.rankToUtil(context.state().computeNextLossRank(), numPlayers)) * BETA_INIT);
			beta = Math.min(beta, ((float) AIUtils.rankToUtil(context.state().computeNextWinRank(), numPlayers)) * BETA_INIT);
		}
	...
	)
	{
		...
		{
			// terminal node
			final double[] utils = AIUtils.utilities(context.state());
		...
		else if (depth == 0)
		{
			...
			final double[] utils = (context.state().numActive() == numPlayers) ? null : AIUtils.utilities(context.state());
			
			for (int p = 1; p <= numPlayers; ++p)
			{
				if (context.state().active(p))
				{
		...
		final float alpha = Math.max(inAlpha, ((float) AIUtils.rankToUtil(context.state().computeNextLossRank(), numPlayers)) * BETA_INIT);
		final float beta = Math.min(inBeta, ((float) AIUtils.rankToUtil(context.state().computeNextWinRank(), numPlayers)) * BETA_INIT);
